...
function determine_waypoint_indexes()
    ...
    for index = 1, mission:num_commands()-1 do
        ...
        if (mitem:command() == MAV_CMD_DO_AUX_FUNCTION) then
            
            -- p1 is type
            -- p2 is details


            -- p1=300, p2=0/1 -> lidar start/stop
            -- p1=300, p2=2   -> wind_direction decision

            -- p1=301, p2=0 -> start of land1 sequence (PRIMARY location)
            -- p1=301, p2=1 -> corresponding NAV_LAND point
            -- p1=301, p2=2 -> start land2 sequence to jump to for reversed landing of primary location

            -- p1=302, p2=0 -> start of land1 sequence (SECONDARY location)
            -- p1=302, p2=1 -> corresponding NAV_LAND point
            -- p1=302, p2=2 -> start land2 sequence to jump to for reversed landing of secondary location

            if (mitem:param1() == 300) then
                if (mitem:param2() == 0) then
                    wp_lidar_start = index
                elseif (mitem:param2() == 1) then
                    wp_lidar_stop = index
                elseif (mitem:param2() == 2) then
                    wp_decide_landing_direction_index = index
                end
                

            elseif (mitem:param1() == 301) then
                -- landing 1 (primary)
                if (mitem:param2() == 0) then
...
function detect_mission_type(force_gcs_msg)
    ...
    for index = 1, mission:num_commands()-1 do
        ...
        if (mitem:command() == MAV_CMD_DO_AUX_FUNCTION) then
            if (not mission_type_matches_this_script or force_gcs_msg) then
                ...
                gcs:send_text(MAV_SEVERITY_foo, string.format("LUA: mission check DO_AUX_FUNCTION is running"))
            end
            return
        end
    end
    if (mission_type_matches_this_script or force_gcs_msg) then
        ...
        gcs:send_text(MAV_SEVERITY_foo, string.format("LUA: mission check DO_AUX_FUNCTION is snoozing"))
    end
end

gcs:send_text(MAV_SEVERITY_foo, string.format("LUA: SCRIPT START: mission check DO_AUX_FUNCTION"))
