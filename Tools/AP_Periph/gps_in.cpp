...
void Periph_GPS_In::uavcan_handle_fix(const uavcan_equipment_gnss_Fix &msg)
{
    if (seen_fix2) {
        // use Fix2 instead
        return;
...
void Periph_GPS_In::uavcan_handle_fix2(const uavcan_equipment_gnss_Fix2 &msg)
{
    bool process = false;
    seen_fix2 = true;
...
void Periph_GPS_In::uavcan_handle_aux(const uavcan_equipment_gnss_Auxiliary &msg)
{
    if (!isnan(msg.hdop)) {
        seen_aux = true;
...
void Periph_GPS_In::uavcan_handle_heading(const ardupilot_gnss_Heading &msg)
{
...
void Periph_GPS_In::uavcan_handle_status(const ardupilot_gnss_Status &msg)
{
