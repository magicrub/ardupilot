...
#define AP_ADSB_UAVIONIX_ALLOW_USING_AUTOPILOT_BARO             1
...
void AP_ADSB_uAvionix_UCP::send_Transponder_Control()
{
    ...
    if (rx.decoded.transponder_config.messageId != 0 && // config msg has been recieved
        (rx.decoded.transponder_config.baroAltSource == GDL90_BARO_DATA_SOURCE_EXTERNAL) && // HW is configured to allow external baro
        (_frontend._options & uint32_t(AP_ADSB::AdsbOption::Use_Autopilot_GPS_as_External_Baro)) != 0)  // ADSB_OPTION is set
    {
        msg.externalBaroAltitude_mm = _frontend._my_loc.alt * 10; // convert cm to mm
    } else
#endif
    {
