...
function update()
    ...
    if (nav_command == MAV_CMD_NAV_LAND) and (mode == FLIGHT_MODE_PLANE_AUTO) and arming:is_armed() and LANDING_PARAMS_SET == false then
        ...
        assert(param:set('LAND_PITCH_CD',-50), "K1000: Fixed Wing Landing Parameter Failed to Write")
        assert(param:set('TECS_LAND_DAMP',10), "K1000: Fixed Wing Landing Parameter Failed to Write")
        assert(param:set('TECS_LAND_SPDWGT',-1), "K1000: Fixed Wing Preflight Parameter Failed to Write")
