...
void AP_Mount_Trillium::update()
{
    ...
    switch(get_mode()) {
        ...
        case MAV_MOUNT_MODE_RETRACT:
            ...
            if (SRV_Channels::function_assigned(SRV_Channel::k_mount_open)) {
                ...
                if (gimbal_is_pointed_in_stow_orientation) {
                    SRV_Channels::set_output_norm(SRV_Channel::k_mount_open, -1);
                }
        ...
        case MAV_MOUNT_MODE_GPS_POINT:
            if (_roi_target_set) {
                double targetLat = deg2rad(_roi_target.lat * 1.0e-7f);
                double targetLon = deg2rad(_roi_target.lng * 1.0e-7f);
                double targetAlt = _roi_target.alt * 0.01f;    // cm -> m
        ...
        case MAV_MOUNT_MODE_SYSID_TARGET:
            if (_target_sysid_location_set && _target_sysid != 0) {
                SendGeopointCmd(_target_sysid_location, Vector3f(), 0, geopointOptions::geopointNone);
...
void AP_Mount_Trillium::SendGeopointCmd(const Location targetLoc, const Vector3f targetVelNed_vector, const float joystickRange, const geopointOptions options)
{
    ...
    const double targetLat = deg2rad(_target_sysid_location.lat * 1.0e-7f);
    const double targetLon = deg2rad(_target_sysid_location.lng * 1.0e-7f);
    const double targetAlt = _target_sysid_location.alt * 0.01f;    // cm -> m
...
void AP_Mount_Trillium::send_mount_status(mavlink_channel_t chan)
{
    ...
    mavlink_msg_mount_status_send(chan, 0, 0, _current_angle_deg.y, _current_angle_deg.x, _current_angle_deg.z, _mode);
}
