    ...
    for (uint8_t i=1; i<SERIALMANAGER_NUM_PORTS; i++) {
        ...
        if (state[i].uart != nullptr) {
            ...
            switch (state[i].protocol) {
                ...
                case SerialProtocol_Trillium:
                    // Note baudrate is hardcoded to 115200
                    state[i].baud = AP_SERIALMANAGER_TRILLIUM_BAUD;   // update baud param in case user looks at it
                    state[i].uart->begin(state[i].baud,
                                         AP_SERIALMANAGER_TRILLIUM_BUFSIZE_RX,
                                         AP_SERIALMANAGER_TRILLIUM_BUFSIZE_TX);
                    break;
