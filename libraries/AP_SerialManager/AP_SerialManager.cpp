...
const AP_Param::GroupInfo AP_SerialManager::var_info[] = {
    ...
    // @Values: -1:None, 1:MAVLink1, 2:MAVLink2, 3:Frsky D, 4:Frsky SPort, 5:GPS, 7:Alexmos Gimbal Serial, 8:SToRM32 Gimbal Serial, 9:Rangefinder, 10:FrSky SPort Passthrough (OpenTX), 11:Lidar360, 13:Beacon, 14:Volz servo out, 15:SBus servo out, 16:ESC Telemetry, 17:Devo Telemetry, 18:OpticalFlow, 19:RobotisServo, 20:NMEA Output, 21:WindVane, 22:SLCAN, 23:RCIN, 24:MegaSquirt EFI, 25:LTM, 26:RunCam, 27:HottTelem, 28:Scripting, 29:UAV Vision Gimbal
    // @User: Standard
    // @RebootRequired: True
    ...
    for (uint8_t i=1; i<SERIALMANAGER_NUM_PORTS; i++) {
        ...
        if (state[i].uart != nullptr) {
            ...
            switch (state[i].protocol) {
                ...
                case SerialProtocol_UAVVision:
                    // Note baudrate is hardcoded to 115200
                    state[i].baud = AP_SERIALMANAGER_UAVVISION_BAUD;   // update baud param in case user looks at it
                    state[i].uart->begin(state[i].baud,
                                         AP_SERIALMANAGER_UAVVISION_BUFSIZE_RX,
                                         AP_SERIALMANAGER_UAVVISION_BUFSIZE_TX);
                    break;
